...
describe('Compression Client Utilities', () => {
  ...
  beforeAll(() => {
    log = setupLogging('mixed_compression_patterns.log', 'compression/client');
  ...
  it('should handle compression and decompression of data with alternating compression-friendly and compression-resistant patterns', async () => {
    log(
      '\nTesting compression and decompression of data with alternating compression-friendly and compression-resistant patterns...',
    );
    const executionTime = await measureAsyncExecutionTime(async () => {
      const testData = new Uint8Array(10000);
      for (let i = 0; i < testData.length; i++) {
        if (i % 1000 < 500) {
          // Compression-friendly pattern (repeating sequence)
          testData[i] = i % 10;
        } else {
          // Compression-resistant pattern (random bytes)
    ...
    log(
      `Compression and decompression of data with alternating compression-friendly and compression-resistant patterns successful. Execution time: ${executionTime}ms`,
    );
  });
});
