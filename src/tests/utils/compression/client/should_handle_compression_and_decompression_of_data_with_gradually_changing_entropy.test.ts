...
describe('Compression Client Utilities', () => {
  ...
  it('should handle compression and decompression of data with gradually changing entropy', async () => {
    log('\nTesting compression and decompression of data with gradually changing entropy...');
    const executionTime = await measureAsyncExecutionTime(async () => {
      const testData = new Uint8Array(10000);
      for (let i = 0; i < testData.length; i++) {
        const entropy = i / testData.length;
        testData[i] = Math.random() < entropy ? Math.floor(Math.random() * 256) : 0;
      }
    ...
    log(
      `Compression and decompression of data with gradually changing entropy successful. Execution time: ${executionTime}ms`,
    );
  });
