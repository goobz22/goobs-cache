...
describe('Compression Client Utilities', () => {
  ...
  beforeAll(async () => {
    const logFunction = await runTestsWithLogging('incrementing_bytes.log', 'compression/client');
  ...
  it('should handle compression and decompression of data with incrementing bytes', async () => {
    log('\nTesting compression and decompression of data with incrementing bytes...');
    ...
    log(
      `Compression and decompression of data with incrementing bytes successful. Execution time: ${executionTime}ms`,
    );
  });
