...
describe('Compression Client Utilities', () => {
  ...
  beforeAll(() => {
    log = setupLogging('decreasing_entropy.log', 'compression/client');
  ...
  it('should handle compression and decompression of data with decreasing entropy', async () => {
    log('\nTesting compression and decompression of data with decreasing entropy...');
    const executionTime = await measureAsyncExecutionTime(async () => {
      const testData = new Uint8Array(1000);
      for (let i = 0; i < 1000; i++) {
        testData[i] = Math.floor(Math.random() * (256 - i / 4));
      }
    ...
    log(
      `Compression and decompression of data with decreasing entropy successful. Execution time: ${executionTime}ms`,
    );
  });
});