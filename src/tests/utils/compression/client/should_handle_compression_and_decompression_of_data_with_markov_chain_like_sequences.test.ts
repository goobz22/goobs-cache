...
describe('Compression Client Utilities', () => {
  ...
  beforeAll(async () => {
    const logFunction = await runTestsWithLogging('markov_chain.log', 'compression/client');
  ...
  const generateMarkovSequence = (length: number): Uint8Array => {
    const data = new Uint8Array(length);
    const transitionMatrix = Array(256)
      .fill(0)
      .map(() =>
        Array(256)
          .fill(0)
    ...
    let currentState = Math.floor(Math.random() * 256);
    for (let i = 0; i < length; i++) {
      data[i] = currentState;
      const transitions = transitionMatrix[currentState];
      const total = transitions.reduce((sum, val) => sum + val, 0);
      let random = Math.random() * total;
      for (let j = 0; j < 256; j++) {
        random -= transitions[j];
        if (random <= 0) {
          currentState = j;
          break;
        }
  ...
  it('should handle compression and decompression of data with Markov chain-like sequences', async () => {
    log('\nTesting compression and decompression of data with Markov chain-like sequences...');
    const executionTime = await measureAsyncExecutionTime(async () => {
      const testData = generateMarkovSequence(10000);
    ...
    log(
      `Compression and decompression of data with Markov chain-like sequences successful. Execution time: ${executionTime}ms`,
    );
  });
