...
describe('Compression Client Utilities', () => {
  ...
  beforeAll(() => {
    log = setupLogging('exponential_growth.log', 'compression/client');
  ...
  it('should handle compression and decompression of data with exponential growth pattern', async () => {
    log('\nTesting compression and decompression of data with exponential growth pattern...');
    const executionTime = await measureAsyncExecutionTime(async () => {
      const testData = new Uint8Array(1024);
      let value = 1;
      for (let i = 0; i < testData.length; i++) {
        testData[i] = value % 256;
        value = (value * 2) % 257; // Using prime 257 to create a cycle
      }
    ...
    log(
      `Compression and decompression of data with exponential growth pattern successful. Execution time: ${executionTime}ms`,
    );
  });
});