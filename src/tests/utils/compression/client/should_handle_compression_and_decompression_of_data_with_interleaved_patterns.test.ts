...
describe('Compression Client Utilities', () => {
  ...
  beforeAll(async () => {
    const logFunction = await runTestsWithLogging('interleaved_patterns.log', 'compression/client');
  ...
  it('should handle compression and decompression of data with interleaved patterns', async () => {
    log('\nTesting compression and decompression of data with interleaved patterns...');
    const executionTime = await measureAsyncExecutionTime(async () => {
      const pattern1 = new Uint8Array([1, 2, 3, 4, 5]);
      const pattern2 = new Uint8Array([10, 20, 30, 40, 50]);
      const testData = new Uint8Array(10000);
      for (let i = 0; i < testData.length; i++) {
        testData[i] = i % 2 === 0 ? pattern1[i % pattern1.length] : pattern2[i % pattern2.length];
      }
    ...
    log(
      `Compression and decompression of data with interleaved patterns successful. Execution time: ${executionTime}ms`,
    );
  });
