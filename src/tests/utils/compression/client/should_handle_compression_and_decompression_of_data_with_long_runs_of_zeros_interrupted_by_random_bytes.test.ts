...
describe('Compression Client Utilities', () => {
  ...
  beforeAll(async () => {
    const logFunction = await runTestsWithLogging('zeros_with_random.log', 'compression/client');
  ...
  it('should handle compression and decompression of data with long runs of zeros interrupted by random bytes', async () => {
    log(
      '\nTesting compression and decompression of data with long runs of zeros interrupted by random bytes...',
    );
    const executionTime = await measureAsyncExecutionTime(async () => {
      const testData = new Uint8Array(100000); // 100KB
      for (let i = 0; i < testData.length; i++) {
        testData[i] = i % 1000 === 0 ? Math.floor(Math.random() * 256) : 0;
      }
    ...
    log(
      `Compression and decompression of data with long runs of zeros interrupted by random bytes successful. Execution time: ${executionTime}ms`,
    );
  });
