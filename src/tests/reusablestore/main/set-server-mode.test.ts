...
const logStream: WriteStream = createLogStream('set-server-mode-test.log');
const log: (message: string) => void = createLogger(logStream);

describe('Set Server Mode Tests', () => {
  const storeName = 'test-store';
  const mode: CacheMode = 'server';

  beforeAll(() => {
    log('Starting Set Server Mode tests...');
  ...
  it('should set a string value in server mode', async () => {
    ...
    log('Successfully set a string value in server mode');
  });

  it('should set a number value in server mode', async () => {
    ...
    expect(set).toHaveBeenCalledWith(identifier, storeName, value, mode, expirationDate);
    log('Successfully set a number value in server mode');
  });

  it('should set a boolean value in server mode', async () => {
    ...
    expect(set).toHaveBeenCalledWith(identifier, storeName, value, mode, expirationDate);
    log('Successfully set a boolean value in server mode');
  });

  it('should set a JSON value in server mode', async () => {
    ...
    expect(set).toHaveBeenCalledWith(identifier, storeName, value, mode, expirationDate);
    log('Successfully set a JSON value in server mode');
  });
  ...
  it('should handle setting a value with a long expiration date', async () => {
    ...
    expect(set).toHaveBeenCalledWith(identifier, storeName, value, mode, expirationDate);
    log('Successfully set a value with a long expiration date in server mode');
  });
  ...
  it('should handle setting a value with no expiration date', async () => {
    ...
    log('Successfully set a value with no expiration date in server mode');
  });
  ...
  it('should handle setting a large value', async () => {
    ...
    expect(set).toHaveBeenCalledWith(identifier, storeName, value, mode, expirationDate);
    log('Successfully set a large value in server mode');
  });
  ...
  it('should handle errors when setting a value', async () => {
    ...
    (set as jest.Mock).mockRejectedValueOnce(new Error('Failed to set value on server'));

    await expect(set(identifier, storeName, value, mode, expirationDate)).rejects.toThrow(
      'Failed to set value on server',
    );

    log('Successfully handled errors when setting a value in server mode');
  });
  ...
  it('should handle setting multiple values in quick succession', async () => {
    ...
    log('Successfully set multiple values in quick succession in server mode');
  });
});
