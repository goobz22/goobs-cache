...
describe('Compression Client Utilities', () => {
  ...
  beforeAll(() => {
    log = setupLogging('abrupt_changes.log', 'compression/client');
  ...
  it('should handle compression and decompression of data with abrupt changes', async () => {
    log('\nTesting compression and decompression of data with abrupt changes...');
    const executionTime = await measureAsyncExecutionTime(async () => {
      const testData = new Uint8Array(1000);
      for (let i = 0; i < 1000; i++) {
        testData[i] = i < 500 ? 0 : 255;
      }
    ...
    log(
      `Compression and decompression of data with abrupt changes successful. Execution time: ${executionTime}ms`,
    );
  });
});
