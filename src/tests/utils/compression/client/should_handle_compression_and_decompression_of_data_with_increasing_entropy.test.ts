...
describe('Compression Client Utilities', () => {
  ...
  beforeAll(async () => {
    const logFunction = await runTestsWithLogging('increasing_entropy.log', 'compression/client');
  ...
  it('should handle compression and decompression of data with increasing entropy', async () => {
    log('\nTesting compression and decompression of data with increasing entropy...');
    const executionTime = await measureAsyncExecutionTime(async () => {
      const testData = new Uint8Array(1000);
      for (let i = 0; i < 1000; i++) {
        testData[i] = Math.floor(Math.random() * (i / 4 + 1));
      }
    ...
    log(
      `Compression and decompression of data with increasing entropy successful. Execution time: ${executionTime}ms`,
    );
  });
