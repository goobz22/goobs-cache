...
describe('Compression Client Utilities', () => {
  ...
  beforeAll(async () => {
    const logFunction = await runTestsWithLogging('periodic_noise.log', 'compression/client');
  ...
  it('should handle compression and decompression of data with periodic noise', async () => {
    log('\nTesting compression and decompression of data with periodic noise...');
    const executionTime = await measureAsyncExecutionTime(async () => {
      const testData = new Uint8Array(10000);
      for (let i = 0; i < testData.length; i++) {
        const baseValue = i % 256;
        const noise = Math.sin(i * 0.1) * 10;
        testData[i] = Math.max(0, Math.min(255, Math.floor(baseValue + noise)));
      }
    ...
    log(
      `Compression and decompression of data with periodic noise successful. Execution time: ${executionTime}ms`,
    );
  });
