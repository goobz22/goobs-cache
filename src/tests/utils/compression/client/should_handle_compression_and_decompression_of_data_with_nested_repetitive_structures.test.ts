...
describe('Compression Client Utilities', () => {
  ...
  beforeAll(async () => {
    const logFunction = await runTestsWithLogging('nested_structures.log', 'compression/client');
  ...
  const generateNestedStructure = (depth: number, length: number): Uint8Array => {
    if (depth === 0) {
      return new Uint8Array(length).fill(Math.floor(Math.random() * 256));
    }
    const subStructure = generateNestedStructure(depth - 1, length / 2);
    const result = new Uint8Array(length);
    for (let i = 0; i < length; i += subStructure.length) {
      result.set(subStructure, i);
    }
    return result;
  };

  it('should handle compression and decompression of data with nested repetitive structures', async () => {
    log('\nTesting compression and decompression of data with nested repetitive structures...');
    const executionTime = await measureAsyncExecutionTime(async () => {
      const testData = generateNestedStructure(5, 32768); // 32 KB of nested structures
    ...
    log(
      `Compression and decompression of data with nested repetitive structures successful. Execution time: ${executionTime}ms`,
    );
  });
